...
split<-sample(nrow(bank),0.80*nrow(bank),replace = F)
train<-bank[split,]
test<-bank[-split,]
...
bank_add_logreg_roc <- prediction(bank_glm_pred, test$y)
rocr.perf_glm=performance(bank_add_logreg_roc, "tpr", "fpr")
bank_add_logreg_auc <- performance(bank_add_logreg_roc, "auc")
auc_glm<-bank_add_logreg_auc@y.values[[1]]
