...
bank %>% 
  select(age, duration, campaign, emp.var.rate, cons.price.idx, cons.conf.idx, euribor3m, nr.employed,y) %>% 
  cor() %>% 
  corrplot(method = "pie",
           type = "upper",
           tl.cex = 0.8,
...
# Scatter Plot Matrix
bank_matrix_factor1 <- bank %>% select(job, marital, education, default, housing)
pairs(bank_matrix_factor1)

bank_matrix_factor2 <-bank %>% select(loan,
